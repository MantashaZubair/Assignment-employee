        ...
        <div className="profile-card">
          ...
          <div className="contentbox">
            ...
            { auth.dob && <span>
                ...
                {auth.dob ?auth.dob.slice(0,10) : 'dob'}
              </span>}
              { auth.location && <span>
                <MdOutlineLocationOn className="profileicon" />
                {auth.location || "location_on"}
              </span>}
            </div>
            {auth.salary&&<p>Salary: {auth.salary ? auth.salary : 'Restricted'}</p>}
            {departmentName&&<p>Department: {departmentName || 'Restricted'}</p>}
            <p>Join Date: {auth.createdAt.slice(0,10)}</p>
          </div>
